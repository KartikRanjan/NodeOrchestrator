...
describe('NodeService', () => {
    ...
    describe('disconnectNode', () => {
        ...
        it('throws 404 when node is not found', async () => {
            ...
            await expect(service.disconnectNode('unknown'))
                .rejects.toMatchObject({ statusCode: 404, errorCode: 'NODE_NOT_FOUND' });
        });

        it('throws validation error when nodeId is empty', async () => {
            await expect(service.disconnectNode(''))
                .rejects.toMatchObject({ statusCode: 400 });
        });
    });
    ...
    describe('getNodeById', () => {
        ...
        it('throws 404 when node does not exist', async () => {
            ...
            await expect(service.getNodeById('ghost'))
                .rejects.toMatchObject({ statusCode: 404, errorCode: 'NODE_NOT_FOUND' });
        });
    });
